...
if (isset($_POST['submit'])) {  
    ...
    if ($file_error === 4) {
        header("Location: ../2_adminUAddProduct.php?upload=nofileupload");
    }
    ...
    if(!in_array($image_ext, $image_ext_filter)) {
        header("Location: ../2_adminUAddProduct.php?upload=yournotuploadingimages");
    }
    ...
    if ($inserting_query) {
        header("Location: ../2_adminUAddProduct.php?upload=success");
    } else {
        header("Location: ../2_adminUAddProduct.php?upload=failed");
    }
}